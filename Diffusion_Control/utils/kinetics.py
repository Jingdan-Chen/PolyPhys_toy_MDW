...
def M_kinetic(It, R_arr, P_arr, M1, config):
    ...
    ki = config['ki']
    f = config['f']
    ktd_temp = config['kt'] * config['ktd_ratio']
    ktc_temp = config['kt'] * (1 - config['ktd_ratio'])
    ktd = ktd_temp if not config['flag5'] else ktd_temp + ktc_temp
...
def R1_kinetic(It, R_arr, P_arr, M1, config):
    ...
    ktd_temp = config['kt'] * config['ktd_ratio']
    ktc_temp = config['kt'] * (1 - config['ktd_ratio'])
    ktd = ktd_temp if not flag5 else ktd_temp + ktc_temp
    ktc = ktc_temp if not flag5 else 0
    kfm = config['kfm'] if flag3 else 0
    kit = config['kit'] if flag1 else 0
...
def get_Vf(M1, P_arr, config, Vf0=.025):
    alpha_m = config['alpha_m']
    alpha_p = config['alpha_p']
    Tg_m = config['Tg_m']
    Tg_p = config['Tg_p']
    
    rou_m = config['density']
    rou_p = config['density_p']
    
    P_sum = np.dot(P_arr, np.linspace(1, len(P_arr) + 1, len(P_arr)))
    V_frac_m = (M1 / rou_m) / (M1 / rou_m + P_sum / rou_p)
    V_frac_p = 1 - V_frac_m
    
    Vf_m = (Vf0 + alpha_m * (config['T'] + 273.15 - Tg_m)) * V_frac_m
    Vf_p = (Vf0 + alpha_p * (config['T'] + 273.15 - Tg_p)) * V_frac_p
    Vf = Vf_m + Vf_p
    
    return Vf

def get_kt_seg_factor(P_arr, config):
    delta = config['delta']
    
    P_sum = np.dot(P_arr, np.linspace(1, len(P_arr) + 1, len(P_arr)))
    if np.isnan(P_sum):
        P_sum = 0
    return 1 + delta * P_sum * config['Mw']

def get_krd(M1, P_arr, config):
    P_sum = np.dot(P_arr, np.linspace(1, len(P_arr) + 1, len(P_arr)))
    if np.isnan(P_sum):
        P_sum = 0
    P_n_avg = P_sum / P_sum.sum()
    if np.isnan(P_n_avg):
        P_n_avg = 0
    convergence = P_sum / (P_sum + M1)
    density = config['density'] * (1 - convergence) + config['density_p'] * convergence
    
    Vm = 1/(config['Mw'] * density)
    
    
    delta = np.power(6*Vm/np.pi/NA, 1/3) 
    D = P_n_avg * (config['l0'] **2) / 6
    
    return 8 * np.pi * delta * D * NA / 1000
    
def get_kt_trans_factor(M1, P_arr, Mw_cr, config):
    Vfc = config['Vfc']
    A = config['A']
    n = config['n']
    P_sum = np.dot(P_arr, np.power(np.linspace(1, len(P_arr) + 1, len(P_arr)), 2))
    Mw = P_sum / np.sum(P_arr * np.linspace(1, len(P_arr), len(P_arr))) * config['Mw']
    Vf = get_Vf(M1, P_arr, config)
    
    return np.minimum(1., ((Mw_cr / Mw) **n)) * np.exp(-A*(1/Vf - 1/Vfc))
    
     
def evolution(It, M1, R_arr, P_arr, dt, t, config):
    # print(config['Mw_cr'])
    if config['Mw_cr'] is None:
        Mw_avg = np.dot(P_arr, np.power(np.linspace(1, len(P_arr) + 1, len(P_arr)), 2)) / P_arr.sum() * (config['Mw'] ** 2)
        if np.isnan(Mw_avg):
            Mw_avg = 0
        Vf = get_Vf(M1, P_arr, config)
        config['kt'] = get_kt_seg_factor(P_arr, config) * config['kt0'] + get_krd(M1, P_arr, config)
        
        if Vf < config['Vfc']:
            print("Shifting to transport diffusion control!!!")
            config['Mw_cr'] = Mw_avg
            config['kt_cr'] = config['kt']
    else:
        config['kt'] = get_kt_trans_factor(M1, P_arr, config['Mw_cr'], config) * config['kt_cr'] + get_krd(M1, P_arr, config)
        
    # print(f"Vf: {Vf}, Mw_avg: {Mw_avg}, kt: {config['kt']}, kt_cr: {config['kt_cr']}")
    ...
    return new_It, new_M, new_R_arr, new_Pn, t + dt, config
