...
# def get_ktd(kt=None, T=T_exp):
#     kt = get_kt(T) if kt is None else kt
#     return kt * get_ktd_ratio(T)  # L/mol/min

# def get_ktc(kt=None, T=T_exp):
#     kt = get_kt(T) if kt is None else kt
#     return kt * (1 - get_ktd_ratio(T))  # L/mol/min
...
def get_Cpp():
    return 400.
def get_density_p(T=T_exp):
    # 1.084-0.000605 (T-273.15)
    return 1.084 - 0.000605 * (T - T0)  # g/cm^3
def get_delta():
    return 0.001

def get_Vfc(T=T_exp):
    # 0.31105 exp((-1671.8/RT))
    return 0.31105 * math.exp(-1671.8/R/T)  # L/mol/min
...
def get_m():
    return 0.5
def get_K3(T=T_exp):
    # 9.44 exp((-3832.9/RT)^3)
    return 9.44 * math.exp(-3832.9/R/T)  # L/mol/min
def get_l0():
    return 7.4E-8  # cm